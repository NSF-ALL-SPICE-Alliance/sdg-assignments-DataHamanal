...
```{r}
goal_4_histogram <- ggplot(sdr_data, aes(x = goal_4_score, y = goal_6_score, fill=regions_used_for_the_sdr)) +
  geom_point() +
  theme_dark() +
...
```{r, warning = FALSE}
goal_4_scores <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, fill = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d(option = "inferno") +
  labs(title = "Distributions of SDG 4 Scores",
...
ggplotly(goal_4_scores)
```


```{r}
goal_3_goal_4_scatter_plot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr, lable = country)) +
  geom_point() +
  theme_dark() +
  scale_color_viridis_d(option = "AsteroidCity2") +
  labs(title = "Distributions of SDG 4 Scores",
       x = "Goal 3 score",
       y = "Goal 4 score",
       color = "Region")

ggplotly(goal_3_goal_4_scatter_plot)
```
```{r}
library("viridis")
```
...
```{r}
goal_3_goal_4_scatter_plot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point(aes(color = "Dark2")) +
  geom_smooth(aes(color = "Dark2", fill = "Dark2"), method = "lm") + 
...
mytext <- paste(
    ...
    "Goal 8 Score: ", round(sdr_data_world_joined$goal_8_score, 2), 
    sep="") %>%
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 5.5, smoothFactor = .5, color = ~colorQuantile("Dark2", goal_8_score)(goal_8_score), label = mytext)
```
