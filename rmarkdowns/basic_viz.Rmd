...
```{r}
ggplot(data = sdr_data, aes(x= goal_6_score, y = goal_4_score)) +
  geom_hex()
```
...
```{r}
ggplot(data = sdr_data, aes(x= goal_4_score, y = goal_6_score, color = regions_used_for_the_sdr)) +
  geom_hex()
```
...
```{r}
ggplot(data = sdr_data, aes(x= goal_11_score, y = goal_6_score, color = goal_9_score)) +
  geom_hex()
```
...
```{r}
ggplot(data = sdr_data, aes(x = goal_7_score, y = goal_13_score, color = goal_9_score)) + geom_point()
```
This graph shows that countries with high Goal 9 scores also have a low Goal 7 score.
...
```{r}
ggplot(MENA_sdr_data, aes(x= goal_11_score, y = country)) +
...
ggplot(sdr_data, aes(x= goal_3_score, y = country)) +
  ...
  theme(axis.text.y = element_text(size = 8)) # adjusting text size of y-axis values
```
...
```{r}
ggplot(sdr_data, aes(x = goal_16_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="yellow")
```
